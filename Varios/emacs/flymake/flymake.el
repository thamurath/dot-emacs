...
(defcustom flymake-allowed-file-name-masks '((".+\\.c$" 1) (".+\\.cpp$" 1) (".+\\.cc$" 1) (".+\\.java$" 3)
											 (".+\\.h$" 2 (".+\\.cpp$" ".+\\.c$" ".+\\.cc$")
...
(defun flymake-find-possible-master-files(file-name master-file-dirs)
    "find (by name and location) all posible master files, which are .cpp .cc and .c for and .h.
...
(defun flymake-master-file-compare(file-one file-two)
    "used in sort to move most possible file names to the beginning of the list (File.h -> File.cpp and .cc moved to top"
